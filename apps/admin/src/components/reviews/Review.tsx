...
const ReviewContent = ({ name, description, rating }: ReviewContentProps) => (
  <>
    <div className="mb-3">
...
export const Review = ({ review, locale, editDialogHandle }: ReviewProps) => {
    ...
    <div className="group relative">
      {url ? (
        <a
          ...
          className="flex h-full min-w-0 shrink-0 grow-0 basis-[75%] xs:basis-[45%] flex-col justify-between rounded-md border border-primary-light bg-white p-5 no-underline hover:no-underline md:basis-[40%] lg:basis-[calc(33.1%-8px)] dark:border-tertiary-light dark:bg-tertiary"
        >
          <ReviewContent rating={rating} name={name} description={description} />
        </a>
      ) : (
